...
class Hp_getter():
    def __init__(self):
        ...
        for i in hModule:
          ...
          if temp[-8:] == "mono.dll":
            self.mono = i.value
        print("Mono", self.mono, "UnityPlayer", self.UnityPlayer)
    
    def get_souls(self):
        base_address = self.UnityPlayer + 0x00FA0998
        offset_address = ctypes.c_void_p()
    ...
    def get_boss_hp(self):
        ...
        #print(self.UnityPlayer)
        #print(0x00FEF994)
        #print(int(self.process_handle), base_address, ctypes.byref(offset_address))
        self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(base_address), ctypes.byref(offset_address), 4, None)
        for offset in offset_list:
          self.kernal32.ReadProcessMemory(int(self.process_handle), offset_address.value + offset, ctypes.byref(offset_address), 4, None)
        
        print("bosshp", offset_address.value)
    ...
    def get_play_location(self):
        ...
        for offset in offset_list:
          self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(x.value + offset), ctypes.byref(x), 4, None)
        xx = ctypes.c_float()
        self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(x.value + 0x44), ctypes.byref(xx), 4, None)
        ...
        for offset in offset_list:
          self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(y.value + offset), ctypes.byref(y), 4, None)

        yy = ctypes.c_float()
        self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(y.value + 0xC), ctypes.byref(yy), 4, None)

        print(xx.value, yy.value)
    ...
    def get_hornet_location(self):
        ...
        self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(base_address), ctypes.byref(x), 4, None)
        for offset in offset_list:
          self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(x.value + offset), ctypes.byref(x), 4, None)
          
        xx = ctypes.c_float()
        self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(x.value + 0xC), ctypes.byref(xx), 4, None)

        base_address = self.UnityPlayer + 0x00FEF994
        y = ctypes.c_void_p()
        offset_list = [0x54, 0x8, 0x1C, 0x1C, 0x14]
        self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(base_address), ctypes.byref(y), 4, None)
        for offset in offset_list:
          self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(y.value + offset), ctypes.byref(y), 4, None)
     
        yy = ctypes.c_float()
        self.kernal32.ReadProcessMemory(int(self.process_handle), ctypes.c_void_p(y.value + 0xAC), ctypes.byref(yy), 4, None)

        if xx.value > 14 and xx.value < 40:
          self.hx = xx.value
        print("hornet:", xx.value, yy.value)
